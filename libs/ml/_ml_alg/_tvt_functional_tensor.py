...
def to_tensor(x: Union[ndarray, Image.Image]) -> Tensor:
    """
    x: ndarray[H, W, C] or [H, W] uint8/Any. Image.Image["RGB", "L"]
    return: Tensor[C, H, W] or [1, H, W]. 
        if x is uint8, 则输出除以255 -> [0..1]
    """
    if isinstance(x, Image.Image):
        if x.mode not in {"RGB", "L"}:  # 这里只支持RGB
...
if __name__ == "__main__":
    ...
    y = ml.test_time(lambda: tvtF.resize(x, 100, tvtF.InterpolationMode.BILINEAR, 120))
    y2 = ml.test_time(lambda: resize(x, 100, tvtF.InterpolationMode.BILINEAR, 120))
    print(torch.allclose(y, y2), y.shape)
